...
void Rebalance(Root root, Node *grandson){
    Node *grandpa = grandson->father->father;
    Node *dad = grandson->father;
    Node *aux;

    if(root->first == grandson){
        return;
    }
    else{
        if(dad->value < grandpa->value){
            if(grandpa->right == NULL){
                grandpa->depth++;
                dad->depth--;
                grandson->depth--;
                aux = grandpa->father;
                if(grandpa->value < aux->value){
                    aux->left = father;
                }
                else{
                    aux->right = father;
