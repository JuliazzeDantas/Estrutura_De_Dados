...
typedef struct Stack { //A estrutura da pilha. Basta apontar para o o topo dela e o resto está ligado
    ...
    int maxSize; //tamanho maximo da pilha
    struct Block *top; // ponteiro que apointa para o topo da pilha
} Stack;

int OverFlow(Stack *prime){ //função para identificar se a pilha já esta totalmente cheia
    if(prime->size == (prime->maxSize)){ //verificação entre o tamanho atual e o tamanho maximo da pilha
        return 1; // retorno 1 (um) em caso de verdadeiro
    }
    else{
        return 0; //retorna 0 (zero) caso seja falso
    }
}
...
int Push(struct Stack *prime, float number){ //função para colocar um novo elemento
    ...
    if(OverFlow(prime)){ //checa se a pilha está cheia
            printf("A pilha ja' esta' cheia. Favor esvazia-la!\n");
        return 0;
    }
...
void Initial(struct Stack *prime, int x){ //inicia a pilha com valores nulos
    prime->size = 0; //zera o tamanho dela
    prime->maxSize = x; //guarda o valor maximo dela
...
void main(){// inicio da main
    ...
    int size; // variavel do tamanho

    float number; //variavel para guardar os numeros que serão inseridos

    printf("Qual o tamanho da pilha: \n");
    scanf("%d", &size); // leitura do tamanho da pilha
    Initial(&x, size); // iniciação da pilha
    ...
    while(confirmation != 0){ // loop de interação com o usuario
        ...
        if(confirmation == 1){
            ...
            case 4:
                if(OverFlow(&x) == 1){
                    printf("A pilha esta lotada \nn");
                }
                else{
                    printf("Faltam %d elemento para a pilha ficar lotada \n", (x.maxSize - x.size));
                }
            break;
